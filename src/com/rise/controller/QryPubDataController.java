...
public class QryPubDataController {
	...
	public void qryClassInstList(String schoolId, String courseType, String stageId, String classType, String classState, String classInstId, HttpServletResponse response) {
		PrintWriter out = null;
		try {
			response.setCharacterEncoding("UTF-8");
			out = response.getWriter();
	...
	@RequestMapping(value = "/qryOnJobTeacherList.do")
	public void qryOnJobTeacherList(String schoolId, String stageId, HttpServletResponse response) {
		PrintWriter out = null;
		try {
			response.setCharacterEncoding("UTF-8");
			out = response.getWriter();
			String retVal = qryPubDataService.qryTeacherList(schoolId, stageId, "P");
			out.write(retVal);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	//判断是否展示片区
	@RequestMapping(value = "/checkStaffIsArea.do")
	public void checkStaffIsArea(String staffId, HttpServletResponse response) {
		PrintWriter out = null;
		try {
			response.setCharacterEncoding("UTF-8");
			out = response.getWriter();
			String retVal = qryPubDataService.checkStaffIsArea(staffId);
			out.write(retVal);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	//通过staffId查询片区
	@RequestMapping(value = "/qryRegionListByStaffId.do")
	public void qryRegionListByStaffId(String staffId, HttpServletResponse response) {
		PrintWriter out = null;
		try {
			response.setCharacterEncoding("UTF-8");
			out = response.getWriter();
			String retVal = qryPubDataService.qryRegionListByStaffId(staffId);
			JSONObject rstObj = JSONObject.fromObject(retVal);
			out.write(StringUtil.getJSONObjectKeyVal(rstObj, "rows"));
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
