...
<html>
  	...
  	<body class="manage">
						...
						<tr>
							<c:choose>
								...
								<c:otherwise>
									...
									<c:choose>
										<c:when test="${fn:length(node.calendarHiddenRealList) > 0}">
											<c:set var="calendarHiddenList" value="${node.calendarHiddenRealList}" />
										</c:when>
										<c:otherwise>
											<c:set var="calendarHiddenList" value="${node.calendarHiddenPlanList}" />
										</c:otherwise>
									</c:choose>
									<c:forEach items='${calendarHiddenList}' var='nodeObj' varStatus="j">
		...
		<table id="classAttendInfo" class="easyui-datagrid" title="" style="height:99%;margin:0px auto;margin-bottom:5px;height:160px;width:99%" pagination="false" rownumbers="false" fitColumns="true" singleSelect="true">
			<thead>
				<tr>
