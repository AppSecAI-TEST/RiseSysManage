$(document).ready(function(){
    ...
    $('#link_data').datagrid({  
	    ...
	    columns:[[  
	        ...
	        {field:'effDate',title:'开始时间',width:'10%',align:'center'},
	        {field:'expDate',title:'结束时间',width:'10%',align:'center'},
	        ...
	        {field:'opt',title:'操作',width:'23%',align:'center',
	            ...
	            {  
	        		if(row.isUse == "Y"){
		                var btn = '<a class="view" onclick="viewLinkPrice(\''+row.setPriceId+'\',\''+row.priceName+'\',\''+row.applySchools+'\',\''+row.effDate+'\',\''+row.expDate+'\',\''+row.isUseVal+'\',\''+row.handerName+'\',\''+row.createDate+'\')" style="width:80px; height:25px;"  href="javascript:void(0)"><span>浏览</span></a>';  
		                return btn;  
	        		}else if(row.isUse == "N"){
	        			var btn = '<a class="view" onclick="viewLinkPrice(\''+row.setPriceId+'\',\''+row.priceName+'\',\''+row.applySchools+'\',\''+row.effDate+'\',\''+row.expDate+'\',\''+row.isUseVal+'\',\''+row.handerName+'\',\''+row.createDate+'\')" style="width:80px; height:25px;"  href="javascript:void(0)"><span>浏览</span></a>&nbsp;<a class="update" onclick="updateLinkPrice(\''+row.setPriceId+'\',\''+row.priceName+'\',\''+row.schoolIds+'\',\''+row.applySchools+'\',\''+row.effDate+'\',\''+row.expDate+'\',\''+row.isUseVal+'\',\''+row.handerName+'\',\''+row.createDate+'\')" style="width:80px; height:25px;"  href="javascript:void(0)"><span>修改</span></a>&nbsp;<a class="delete" onclick="deleteLinkPrice(\''+row.setPriceId+'\')" style="width:80px; height:25px;"  href="javascript:void(0)"><span>删除</span></a>';  
		                return btn;  
	        		}else if(row.isUse == "X"){
	        			var btn = '<a class="view" onclick="viewLinkPrice(\''+row.setPriceId+'\',\''+row.priceName+'\',\''+row.applySchools+'\',\''+row.effDate+'\',\''+row.expDate+'\',\''+row.isUseVal+'\',\''+row.handerName+'\',\''+row.createDate+'\')" style="width:80px; height:25px;"  href="javascript:void(0)"><span>浏览</span></a>';  
		                return btn;  
	   ...
	   var tabs = $(".tabs").height();
				...
				$('#link_data').datagrid('resize', {// 适配高度
					height: height - header - tabs*3
				});
});
