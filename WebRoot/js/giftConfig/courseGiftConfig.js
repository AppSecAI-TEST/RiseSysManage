	        ...
	        {field:'opt',title:'操作',width:'13%',align:'center',
	            ...
	            {  
	        		if(row.typeFlag == "true" || row.giftName == ""){
		                ...
		                var btn = '<a class="update" onclick="updateCourseName(\''+row.giftId+'\',\''+row.giftType+'\',\''+row.giftName+'\',\''+row.amount+'\',\''+row.giftNum+'\',\''+row.isEff+'\')" style="width:60px; height:25px;"  href="javascript:void(0)"><span>修改</span></a>&nbsp;<a class="delete" onclick="removeCourseName(\''+row.giftId+'\')" style="width:60px; height:25px;" href="javascript:void(0)"><span>删除</span></a>';  
		                return btn;  
	        		}
	            }  
	        }  
