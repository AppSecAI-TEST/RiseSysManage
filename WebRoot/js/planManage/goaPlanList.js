...
function showAdd()
{
	$("#dlg").dialog('open').dialog('setTitle', '新增目标');
    $('#fm').form('clear');
    $("#submitBtn").unbind();
...
function add()
{
	if(checkParam())
	{
		...
		$.ajax({
			...
			async : false,
			beforeSend : function() {
				showProgressLoader("正在添加目标,请稍等...", 400);
			},
			success : function(data) 
			{
				...
				if (data == "true")
				{
					showMessage('提示', "添加目标成功！", function() 
					{
						hideMessage();
						$("#dlg").dialog('close');
			...
			{
				hideProgressLoader();
				showMessage('提示', "调用添加目标服务失败！", null);
			}
		});
	}
}
...
function showUpdate()
{
	if(validateSelect())
	{
		...
		$("#dlg").dialog('open').dialog('setTitle', '修改目标');
	    $("#submitBtn").unbind();
	    $("#submitBtn").click(function(){
	    	update();
	    });
...
function update()
{
	if(checkParam())
	{
		...
		$.ajax({
			...
			async : false,
			beforeSend : function() {
				showProgressLoader("正在修改目标,请稍等...", 400);
			},
			success : function(data) 
			{
				hideProgressLoader()
				if (data == "true")
				{
					showMessage('提示', "修改目标成功！", function() 
					{
						hideMessage();
						$("#dlg").dialog('close');
						$("#qryBtn").trigger("click");
...
function del()
{
	if(validateSelect())
	{
		...
		$.messager.confirm('提示','您确定要删除当前选中的目标吗？',function(r)
		{
			if(r)
			{
				$.post('/sys/planManage/deletePlan.do', {planId : planId}, function(result) 
				{
					if(result=="true") 
					{
						$.messager.alert('提示',"删除目标成功");
						$("#qryBtn").trigger("click");
					} 
					else 
					{
						$.messager.alert('提示', "删除目标失败");
					}
				});
			}
		});
