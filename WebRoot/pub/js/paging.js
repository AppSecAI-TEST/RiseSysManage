...
function qryStartFunc(obj) {
	...
	if (obj.length > 2) {
		obj += ",\"curId\":0,\"pageNum\":" + gpagingCount + "}";
	} else {
		obj += "\"curId\":0,\"pageNum\":" + gpagingCount + "}";
	}
	return obj;
...
function rotPaging(val) {
	if (val == 1) 
	{
		...
		{
			parent.showMessage('当前已经是第一页!');
		}	
	} 
	else 
	{
		if(gcurTag<parseInt($("#totalPage").html()))
		{
			showList(parseInt(gcurTag) + 1);
		}
		else
		{
			parent.showMessage('当前已经是最后一页!');
		}
	}
}
...
function showNode(val) {
	...
	$(cnt).css("color","grey");
	$(cnt).css("background","#ffffff");
}

function hiddenNode(val) {
	var cnt = document.getElementById("cnt_" + val);
	$(cnt).css("cursor","pointer");
	$(cnt).css("color","#ffffff");
	$(cnt).css("background","#38B5E1");
}
...
//	obj.style.backgroundImage = "url('/sysManage/pub/images/btn2.png')";
//}
//
//function backOut(obj) {
//	obj.style.backgroundImage = "url('/sysManage/pub/images/btn1.png')";
//}
